...
hhp <- husown %>%
  left_join(phhold)
saveRDS(hhp, "./data/hhp20175year.rds")
...
hhp2 %>%
  group_by(stabbr, st) %>%
  summarise(n=n(), hsum=sum(wgtp), psum=sum(pwgtp),
            rhincp=sum(wgtp * rhincp) / 1e6, 
            rptax=sum(wgtp * rptax) / 1e6) %>%
  mutate(rhincp.avg=rhincp * 1e6 / hsum,
         rptax.avg=rptax * 1e6 / hsum) %>%
  arrange(-rptax.avg) %>%
  kable(digits=0, format.args=list(big.mark=","))


# build age-property tax profiles ----
glimpse(hhp2)
stage.taxval <- hhp2 %>% 
  filter(agep >= 18) %>%
...
taxval <- bind_rows(stage.taxval, us.taxval)
ht(taxval)

# sts <- c("CA", "NY", "NJ", "US")
sts <- globals$case_study_states
taxval %>%
...
f <- function(df){
  mod <- df %>% 
    ...
    lm(rptax ~ age + age2 + age3 + age4, data=.)
...
g <- function(df){
  mod <- df %>% 
    ...
    filter(age >= 18) %>%
    mutate(age2=age^2, age3=age^3, age4=age^4) %>%
    lm(rvalp ~ age + age2 + age3 + age4, data=.)
  c.df <- as_tibble(t(coef(mod)))
  vnames <- names(c.df)
  vnames[1] <- "intercept"
  vnames <- paste0("rval.", vnames)
  c.df <- c.df %>% setNames(vnames)
  return(c.df)
}

rval.us <- hhp2 %>% do(g(.)) %>% mutate(stabbr="US")
rval.df <- hhp2 %>% group_by(stabbr) %>%
  do(g(.)) %>%
  bind_rows(rval.us)
...
st2 <- taxval %>% 
  rename(age=agep) %>%
  left_join(ptx.df) %>% # tx. prefixes
  left_join(rval.df) %>%
  mutate(rptax.est=tx.intercept + 
           tx.age * age +
           tx.age2 * age^2 +
           tx.age3 * age^3 +
           tx.age4 * age^4,
         rvalp.est=rval.intercept + 
           rval.age * age +
           rval.age2 * age^2 +
           rval.age3 * age^3 +
           rval.age4 * age^4) %>%
  select(stabbr, age, rptax, rvalp, rptax.est, rvalp.est) %>%
  gather(variable, value, -stabbr, -age)

glimpse(st2)
taxval %>%
  filter(stabbr=="US") %>%
  ggplot(aes(x=agep, y=rptax)) +
  geom_line() +
  geom_smooth() +
...
p <- hhp2 %>%
  ...
  ggplot(aes(x=agep, y=rptax, colour=stabbr)) +
...
#.. plot rptax by age, case study states ----
glimpse(hhp2)
p <- hhp2 %>%
  filter(stabbr %in% globals$case_study_states, agep>=20) %>%
  ggplot(aes(x=agep, y=rptax, colour=stabbr, weight=wgtp)) +
  geom_smooth(se=FALSE)
...
capt2 <- "Note: Property tax values are smoothed using a generalized additive model"
capt <- paste0(capt1, "\n", capt2)
...
p1 <- data %>%
  ...
  scale_y_continuous(name="Property tax (self-reported)", breaks=seq(0, 10e3, 500), limits=c(0, NA), labels = scales::dollar) +
  theme_bw() +
...
w <- 6.5 * mult; h <- 5 * mult
w <- 8; h <- 11 / 2
ggsave(plot=p1, filename="./results/proptax_byage_css_xus.png", width=w, height=h, units="in")
