...
wc_src <- "Weldon Cooper Center for Public Service, University of Virginia, updated December 2018, https://demographics.coopercenter.org/"
...
ggsave(plot=p2, filename=here::here("figures_in_report", "fertrate_ssa.png"), width=8, height=6, units="in")
...
ggsave(plot=p, filename=here::here("figures_in_report", "lifeexp_ssa.png"), width=8, height=6, units="in")
...
ggsave(plot=p3, filename=here::here("figures_in_report", "uspopshares_subgroups.png"), width=8, height=6, units="in")
...
ggsave(plot=p3, filename=here::here("figures_in_report", "uspopgr_subgroups.png"), width=8, height=6, units="in")
...
ggsave(plot=p2, filename=here::here("figures_in_report", "agedep_un.png"), width=8, height=6, units="in")
...
ggsave(plot=p, filename=here::here("figures_in_report", "income_itax.png"), width=8, height=6, units="in")

```


#### Case-study states income and PIT graph
```{r}
# replace the old polynomial graph with smoothed regression graph
# cps_src <- "Source: Annual Social and Economic (ASEC) supplement to the Current Population Survey, pooled 2017 and 2018"
note <- "Values are weighted means by age, smoothed with local regression."
capt <- paste0("Source: ", cps_src, "\n", note)
...
sts <- c(globals$case_study_states)
...
p1 <- sts_itax %>% 
  filter(stabbr %in% sts, variable=="rincome") %>%
  ungroup %>%
  mutate(stabbr=factor(stabbr, levels=sts)) %>%
  ggplot(aes(x=age, y=smooth, colour=stabbr)) +
  theme_bw() +
...
p2 <- sts_itax %>% filter(stabbr %in% sts, variable=="rtax") %>%
  ungroup %>%
  mutate(stabbr=factor(stabbr, levels=sts)) %>%
  ggplot(aes(x=age, y=smooth, colour=stabbr)) +
  theme_bw() +
...
ggsave("./results/income_and_tax_byage_css_xus_loess.png", ml, width=8, height=11, units="in")
ggsave(plot=ml, filename=here::here("figures_in_report", "income_and_tax_byage_css_xus_loess.png"), 
...
pit_src <- "Author's analysis of (1) data from the Annual Social and Economic (ASEC) supplement to the Current Population Survey, pooled 2017 and 2018, and (2) projections from the Weldon Cooper Center for Public Service, University of Virginia, updated December 2018, https://demographics.coopercenter.org/."
pittab <- function(st){
  enone_tabs %>% 
    filter(stabbr==st) %>%
    mutate(stname=getstname(stabbr)) %>%
...
ggsave(plot=p, filename=here::here("figures_in_report", "pit_demog.png"), width=8, height=6, units="in")
...
ggsave(plot=p, filename=here::here("figures_in_report", "sut_incexp.png"), width=8, height=6, units="in")
...
ggsave(plot=p, filename=here::here("figures_in_report", "hval_ptax.png"), width=8, height=6, units="in")
...
ggsave(plot=p, filename=here::here("figures_in_report", "proptax_byage_css_xus.png"), width=8, height=6, units="in")
...
capt <- "\nSource: Author's analysis of projections from University of Virginia, Weldon Cooper Center for Public Service, updated December 2018, https://demographics.coopercenter.org/"
p <- popshares %>% 
...
ggsave(plot=p2, filename=here::here("figures_in_report", "age65p_2040_scatter_repel2.png"),
...
wcnote <- "Author's analysis of projections from University of Virginia,\nWeldon Cooper Center for Public Service, updated December 2018, https://demographics.coopercenter.org/"
...
```{r hx_bar, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE}
hbar <- read_csv(
"age_group, hxpc
0-18, 3552
19-44, 4458
...
ggsave(plot=p, filename=here::here("figures_in_report", "maxpe.png"), width=8, height=6.5, units="in")
