...
wmdnage <- readRDS("./data/popproj/wc_mdnage.rds") # Weldon Cooper pop projections

cps_person <- readRDS("./data/cps_person.rds") # CPS person data for 2017-2018

uspp_un <- readRDS("./data_raw/UnitedNations/un_popproj.rds")

hhp <- readRDS("./data/ACS/acs_hhp20175year.rds") # household-person file, ACS
...
vorder <- c("agi", "wages", "retinc", "txblpension", "iradist", "txblsocsec", "non_wage_cg_retagi", "netcgll")
soichanges <- soidat %>% 
...
#.. quick check of implied pit in the cps vs iit in census -- years don't match quite correctly of course
sgcomp <- pit_shares %>%
  group_by(stabbr) %>%
...
```{r}
basic_cex %>%
  kable(caption="CEX expends, different laws", digits=0, format.args=list(big.mark=","))
```


### U.S. plot of income, goods expends, sales tax base
...
### New Hampshire
```{r NH, echo=FALSE, message=FALSE, warning=FALSE}

```


### New York
```{r NY, echo=FALSE, message=FALSE, warning=FALSE}

```


### Ohio
```{r OH, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
### Tennessee
```{r TN, echo=FALSE, message=FALSE, warning=FALSE}
...
### Texas
```{r TX, echo=FALSE, message=FALSE, warning=FALSE}
